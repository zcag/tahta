...
import { rmSync, mkdirSync, copyFileSync, writeFileSync, readdirSync, existsSync } from 'node:fs'
...
// gallery = the explorer; every other examples/* with a slides.md = a live deck (auto-discovered,
// so a new example deck deploys without editing this script).
build('examples/gallery/slides.md', '/gallery/', 'gallery')
const decks = readdirSync(join(root, 'examples'))
  .filter(d => d !== 'gallery' && existsSync(join(root, 'examples', d, 'slides.md')))
  .sort()
for (const d of decks) build(`examples/${d}/slides.md`, `/decks/${d}/`, `decks/${d}`)
...
const urls = ['/', '/gallery/', ...decks.map(d => `/decks/${d}/`)]
writeFileSync(join(dist, 'sitemap.xml'),
  `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n` +
  urls.map(u => `  <url><loc>${SITE}${u}</loc></url>`).join('\n') + `\n</urlset>\n`)

console.log(`\n✓ site → dist/  (landing + gallery explorer + ${decks.length} example decks: ${decks.join(', ')})`)
